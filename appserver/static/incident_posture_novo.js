...
require([
    ...
    $('body').append(`<div class="incident-menu-arrow" data-popdown-role="arrow"></div>
                ...
                <ul class="incident-menu">
                    <li><a href="#" class="incident-menu-item" data-event-action="doexternalworkflowaction">Abre TT no Siebel</a></li>
                </ul>
                </div>`);
    ...
    $('<button />').addClass('btn').addClass('btn-primary').attr('id', 'create_new_incident_button').attr('data-toggle', 'modal').attr('data-target', '#create_new_incident_modal').text('Novo Incidente').appendTo($('div.dashboard-header-editmenu > span'));

    var novo_fields_search = new SearchManager({
        id: "novo_fields_search",
        earliest_time: "-24h@h",
        latest_time: "now",
        preview: true,
        cache: false,
        search: "| inputlookup falha_datacheck.csv" 
    });

    var all_coments = null;
    var novo_fields_search_results = novo_fields_search.data("results",  {output_mode: 'json_rows', count: 0 }); // get the data from that search
    // When data arrives:
    novo_fields_search_results.on("data", function() {
        all_coments = novo_fields_search_results.collection().toJSON()
        console.log(all_coments);
    });
    ...
    $('#create_new_incident_button').click(function() {
        ...
        '  <div class="modal-body modal-body-scrolling" style="max-height:500px !important;">' + // class="modal-content" style="overflow-y: scroll;" 
        '    <div class="modal-header">' +
        '      <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>' +
        '      <h4 class="modal-title">Criar Novo Incidente</h4>' +
        '    </div>' +
        ...
        '      </div>' +
        '      <div class="control-group shared-controls-controlgroup">' +
        '        <label for="impact" class="control-label">Impacto:</label>' +
        ...
        '          </div>' +
        '      <div class="control-group shared-controls-controlgroup">' +
        '        <label for="fields" class="control-label">Campos:</label>' +
        '        <div class="controls"><textarea type="text" name="fields" id="fields" class=""></textarea></div>' +
        '      </div>' +
        '      <p class="control-heading">Config Drilldown (Opcional):</p>'+
        '      <div class="control-group shared-controls-controlgroup">' +
        '        <label for="event_search" class="control-label">Busca Incident:</label>' +
        '        <div class="controls"><textarea type="text" name="event_search" id="event_search" class=""></textarea></div>' +
        '      </div>' +
        '      <div class="control-group shared-controls-controlgroup">' +
        '        <label for="earliest_time" class="control-label">Incident + Antigo:</label>' +
        '        <div class="controls"><input type="text" name="earliest_time" id="earliest_time" class=""></input></div>' +
        '      </div>' +
        '      <div class="control-group shared-controls-controlgroup">' +
        '        <label for="latest_time" class="control-label">Ultimo Incident:</label>' +
        ...
        '        <button type="button" class="btn btn-primary" id="modal-create-new-incident" disabled>Criar</button>' +
        '      </div>' +
        '    </div>' +
        '</div>';
        ...
        /*var novoInput = new DropdownView({
            id: "novoInput",
            managerid: "novo_fields_search",
            default: "main",
            labelField: "txt",
            valueField: "cod",
            el: $("div#novoInput")
        ...
        */
        $.each(all_coments, function(key, val) {
            ...
            $("#falha_datacheck").prop("disabled", false);
        });
        
        //---------------

        var all_urgencies = [ "low" ,"medium", "high" ];
        var all_urgencies_pt = [ "baixo" ,"medio", "alto" ];

        $.each(all_urgencies, function(key, val) {
            var port = all_urgencies_pt[key];
            if (val == urgency) {
        ...
        $.each(all_impacts, function(key, val) {
            var port = all_urgencies_pt[key];
            if (val == impact) {
                $('#impact').append( $('<option></option>').attr("selected", "selected").val(val).html(port) )
            } else {
                $('#impact').append( $('<option></option>').val(val).html(port) )
            }
    ...
    var IncidentDetailsExpansionRenderer = TableView.BaseRowExpansionRenderer.extend({
        ...
        setup: function($container,rowData) {
            ...
            $("<h3 />").text('Detalhes').appendTo($container);
            ...
            $("<div/>").text('Carregando...').attr('id', 'loading-bar-details').appendTo($container);
            ...
            var url = splunkUtil.make_url('/splunkd/__raw/services/alert_manager/helpers?action=get_savedsearch_description&savedsearch_name='+alert.value+'&app=alertmanager');
            var desc = "";
...
'          <i class="icon-alert"></i><span id="info_text">Voce esta editando '+ nr_incidents +' incident</span>' +
'        </div>' +
...
'          <div class="control-group shared-controls-controlgroup">' +
'            <label for="urgency" class="control-label">Falha/Data Check:</label>' +
'            <div class="controls"><select name="falha_datacheck" id="falha_datacheck" disabled="disabled"></select></div>' +
'          </div>' +
'      <div class="control-group shared-controls-controlgroup">' +
        '        <label for="nroTT" class="control-label">Nro TT:</label>' +
...
'      <div class="modal-header">' +
'        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>' +
'        <h4 class="modal-title" id="exampleModalLabel">Executa Macro</h4>' +
'      </div>' +
'      <div class="modal-body modal-body-scrolling">' +
'      <h3 class="text-center">DESEJA REALMENTE EXECUTAR ESTA AÇÃO? </h3>' + 
'        <input type="hidden" id="getincidentid" name="getincidentid" value="'+incident_id+'"/>' + 
'      </div>' +
'      <div class="modal-footer">' +
'        <button type="button" class="btn cancel modal-btn-cancel pull-left" data-dismiss="modal">Cancel</button>' +
'        <button type="button" class="btn btn-primary" id="modal-execute">Execute</button>' +
'      </div>' +
'    </div>' +
'</div>';
...
'</div>';
    ...
    $(document).on("click", "#modal-save", function(event){
        ...
        var falha_datacheck = $("#falha_datacheck").val();
        var numero_tt_siebel = $("#numero_tt_siebel").val();
        ...
        var update_entry = { 'incident_ids': incident_ids, 'comment': comment, 'falha_datacheck': falha_datacheck, 'numero_tt_siebel': numero_tt_siebel };
        ...
        if (status != "(unchanged)") {
            ...
            if (status == "closed"){
                update_entry.alert_close_time = Date.now();
            }else{
                update_entry.alert_close_time = 0;
            }
        }
    ...
    $(document).on("click", "#modal-execute", function(event){
         var incident_id = $("#getincidentid").val();
        
        var command  = "|`executeMacro("+incident_id+")`";
       // var comment  = $("#externalworkflowaction_comment").val();
        ...
        manager.on('search:done', function(properties) {
            debugger;
                alert("Executado com successo");
            
                }); 

        manager.on('search:fail', function(properties) {
           alert("Falha na pesquisa:" + properties);
        }); 

        manager.on('search:error', function(properties) {
            alert("Erro na pesquisa:" + properties);
        }); 
