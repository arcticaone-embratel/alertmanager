...
def createIncident(metadata, config, incident_status, sessionKey):
    ...
    entry['alert_time'] = metadata['dataAbertura']
    ...
    if metadata.get('ClienteId') is not None:
        entry['ClienteId'] = metadata['ClienteId']
    else:
        entry['ClienteId'] = 'NAO INFORMADO'

    if metadata.get('Estado') is not None:
        entry['Estado'] = metadata['Estado']
    else:
        entry['Estado'] = 'NAO INFORMADO'
    
    if metadata.get('Cidade') is not None:
        entry['Cidade'] = metadata['Cidade']
    else:
        entry['Cidade'] = 'NAO INFORMADO'

    if metadata.get('Cliente') is not None:
        entry['Cliente'] = metadata['Cliente']
    else:
        entry['Cliente'] = 'NAO INFORMADO'

    if metadata.get('Servico') is not None:
        entry['Servico'] = metadata['Servico']
    else:
        entry['Servico'] = 'NAO INFORMADO'  

    if metadata.get('Solucao') is not None:
        entry['Solucao'] = metadata['Solucao']
    else:
        entry['Solucao'] = 'NAO INFORMADO'

    if metadata.get('extras') is not None:
        entry['extras'] = metadata['extras']
    else:
        entry['extras'] = '-'

    if metadata.get('siebel') is not None:
        entry['siebel'] = metadata['siebel']
    else:
        entry['siebel'] = '-'
    
    if metadata.get('falha_datacheck') is not None:
        entry['falha_datacheck'] = metadata['falha_datacheck']
    else:
        entry['falha_datacheck'] = '-'

    if metadata.get('numero_tt_siebel') is not None:
        entry['numero_tt_siebel'] = metadata['numero_tt_siebel']
    else:
        entry['numero_tt_siebel'] = '-'

    if metadata.get('TipoNegocio') is not None:
        entry['TipoNegocio'] = metadata['TipoNegocio']
    else:
        entry['TipoNegocio'] = 'NAO INFORMADO' 

    if metadata.get('TipoServico') is not None:
        entry['TipoServico'] = metadata['TipoServico']
    else:
        entry['TipoServico'] = 'NAO INFORMADO' 

    if metadata.get('host') is not None:
        entry['host'] = metadata['host']
    else:
        entry['host'] = 'NAO INFORMADO'

    if metadata.get('Localidade') is not None:
        entry['Localidade'] = metadata['Localidade']
    else:
        entry['Localidade'] = 'NAO INFORMADO'

    if metadata.get('AbertoTT') is not None:
        entry['AbertoTT'] = metadata['AbertoTT']
    else:
        entry['AbertoTT'] = 'NAO INFORMADO'

    if metadata.get('alert_close_time') is not None:
        entry['alert_close_time'] = metadata['alert_close_time']
    else:
        entry['alert_close_time'] = 0

    if metadata.get('metric') is not None:
        entry['metric'] = metadata['metric']
    else:
        entry['metric'] = 'NAO INFORMADO'

    if metadata.get('index') is not None:
        entry['index'] = metadata['index']
    else:
        entry['index'] = 'NAO INFORMADO'

    if metadata.get('thresholdSeverityCode') is not None:
        entry['thresholdSeverityCode'] = metadata['thresholdSeverityCode']
    else:
        entry['thresholdSeverityCode'] = 'NAO INFORMADO'
...
def updateIncident(incident_id, metadata, sessionKey):
    ...
    entry['Cliente'] = incidents[0]['Cliente']
    entry['ClienteId'] = incidents[0]['ClienteId']
    entry['Estado'] = incidents[0]['Estado']
    entry['Cidade'] = incidents[0]['Cidade']
    entry['Servico'] = incidents[0]['Servico']
    entry['Solucao'] = incidents[0]['Solucao']
    entry['extras'] = incidents[0]['extras']
    entry['siebel'] = incidents[0]['siebel']
    entry['falha_datacheck'] = incidents[0]['falha_datacheck']
    entry['numero_tt_siebel'] = incidents[0]['numero_tt_siebel']
    entry['TipoNegocio'] = incidents[0]['TipoNegocio']
    entry['TipoServico'] = incidents[0]['TipoServico']
    entry['Localidade'] = incidents[0]['Localidade']
    #entry('AbertoTT') = incidents[0]['AbertoTT']
    #entry('alert_close_time') = incidents[0]['alert_close_time']
    entry['host'] = metadata['host']
    entry['metric'] = metadata['metric']
    entry['index'] = metadata['index']
    entry['thresholdSeverityCode'] = metadata['thresholdSeverityCode']
...
def getImpactFromResults(results, default_impact, incident_id):
    valid_impacts = {"performance", "low", "medium", "high" }
...
def getCustomFieldsFromResults(results, default_customField, incident_id, field):
    if len(results["fields"]) > 0 and field in results["fields"][0]:
        log.debug("Found CustomField field in results, will use tags={} for incident_id={}".format(results["fields"][0][field], incident_id))
        return results["fields"][0][field]
    else:
        log.debug("No CustomField field found in results. Falling back to default_customField={} for incident_id={}".format(default_customField, incident_id))
        return default_customField
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--execute":
        ...
        metadata.update({ 'Cliente': getCustomFieldsFromResults(results, "N/A", incident_id, "Cliente")})
        metadata.update({ 'ClienteId': getCustomFieldsFromResults(results, "N/A", incident_id, "ClienteId")})

        metadata.update({ 'host': getCustomFieldsFromResults(results, "N/A", incident_id, "host")})
        metadata.update({ 'Estado': getCustomFieldsFromResults(results, "N/A", incident_id, "Estado")})
        metadata.update({ 'Cidade': getCustomFieldsFromResults(results, "N/A", incident_id, "Cidade")})
        metadata.update({ 'Servico': getCustomFieldsFromResults(results, "N/A", incident_id, "Servico")})
        metadata.update({ 'Solucao': getCustomFieldsFromResults(results, "N/A", incident_id, "Solucao")})
        metadata.update({ 'extras': getCustomFieldsFromResults(results, "N/A", incident_id, "extras")})
        metadata.update({ 'siebel': getCustomFieldsFromResults(results, "N/A", incident_id, "siebel")})
        metadata.update({ 'falha_datacheck': getCustomFieldsFromResults(results, "N/A", incident_id, "falha_datacheck")})
        metadata.update({ 'numero_tt_siebel': getCustomFieldsFromResults(results, "N/A", incident_id, "numero_tt_siebel")})
        metadata.update({ 'TipoNegocio': getCustomFieldsFromResults(results, "N/A", incident_id, "TipoNegocio")})
        metadata.update({ 'TipoServico': getCustomFieldsFromResults(results, "N/A", incident_id, "TipoServico")})
        metadata.update({ 'Localidade': getCustomFieldsFromResults(results, "N/A", incident_id, "Localidade")})
        metadata.update({ 'AbertoTT': 'Não'})
        metadata.update({ 'alert_close_time': 0})
        metadata.update({ 'metric': getCustomFieldsFromResults(results, "N/A", incident_id, "metric")})
        metadata.update({ 'index': getCustomFieldsFromResults(results, "N/A", incident_id, "index")})
        metadata.update({ 'thresholdSeverityCode': getCustomFieldsFromResults(results, "N/A", incident_id, "thresholdSeverityCode")})
        metadata.update({ 'dataAbertura': getCustomFieldsFromResults(results, "0", incident_id, "dataAbertura")})
